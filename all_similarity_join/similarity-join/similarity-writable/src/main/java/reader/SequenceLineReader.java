...
public class SequenceLineReader extends RecordReader<NullWritable, SequenceWritable> {
    ...
    public boolean nextKeyValue() throws IOException {
        ...
        outV.getSequence().set(in.getCurrentValue().getBytes(), 0, in.getCurrentValue().getLength());
        if (outV.getLength() == 0)
            return nextKeyValue();
        try {
            outV.translate();
